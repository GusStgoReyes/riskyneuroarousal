...
for (file in asc_files[9:12]) {
  ...
  eye_preproc$timeseries <- eye_preproc$timeseries %>%
    ...
    summarise(
      ...
      pupil_raw_deblink_detransient_interpolate_lpfilt_z = mean(pupil_raw_deblink_detransient_interpolate_lpfilt_z, na.rm = TRUE),
  ...
  eye_preproc$info$sample.rate <- target_hz
  eye_preproc$timeseries$hz <- target_hz
  eye_preproc$timeseries$sub <- subject_num
  eye_preproc$timeseries$run <- run_num


  # (10) Select the relevant columns to save
  output_data <- eye_preproc$timeseries %>%
    select(sub, run, time_orig, pupil_raw_deblink_detransient_interpolate_lpfilt_z, blink, outofbounds)

  output_filename <- paste0(subject_id, "_", run_id, "_timeseries_start2.csv")
