...
class FIRDeconvolution(object): 
    ...
    def betas_for_events(self):
        ...
        for i, covariate in enumerate(self.covariates.keys()):
            if len(self.betas_for_cov(covariate).shape) == 1:
                self.betas_per_event_type[i, :] = self.betas_for_cov(covariate)[:, np.newaxis]
            else:
    ...
    def calculate_rsq(self):
        ...
        if len(explained_signal.shape) == 1:
            explained_signal = explained_signal[np.newaxis, :]
