...
def ddm(
    ...
    model = pyddm.gddm(
        starting_position=response_bias_func,
        drift=drift_func,
...
def drift_function(model_ID):
    if model_ID == 1:
        ...
        return (
            lambda alpha, drift_gain, drift_loss, gain, loss: alpha
            + drift_gain * gain
            - drift_loss * loss
        )
    elif model_ID == 2:
        ...
        return (
            lambda drift_gain, drift_loss, gain, loss: drift_gain * gain
            - drift_loss * loss
        )
    else:
...
if __name__ == "__main__":
    ...
    with open(
        f"{args.results_pth}/sub{args.subj_ID}_model{args.model}.pkl", "wb"
    ) as file:
    ...
    pd.DataFrame(params_save).to_csv(
        f"{args.results_pth}/sub{args.subj_ID}_model{args.model}.csv"
    )
