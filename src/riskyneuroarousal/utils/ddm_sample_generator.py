...
def drift_func(drift, c, alpha, lambd, gain, loss):
    dv = np.power(gain, alpha) - lambd * np.power(loss, alpha)
    if dv > 0:
        dv = np.power(dv, 1 / alpha)
    else:
        dv = -np.power(-dv/lambd, 1 / alpha)
    return drift*dv - c

def get_ddm_model(
    x0=0.5, alpha=1, nondecision=0.3, drift_gain=1, drift_loss=1, bound=1, theta=0.6
):
    model = pyddm.gddm(
...
def get_ddm_model_2(
    x0=0.5, drift = 1, c = 1, alpha = 0.9, lambd = 1.1, nondecision=0.3, bound=1, theta=0.6
):
    model = pyddm.gddm(
        starting_position=x0,
        drift=lambda gain, loss: drift_func(drift, c, alpha, lambd, gain, loss),
        nondecision=nondecision,
        bound= lambda t: bound - theta * t,
        conditions=['gain', 'loss'],
        T_dur=4,
...
def model_behavior(model_ID):
    ...
    for sub in data["sub"].unique():
        ...
        if model_ID == 2:
            ...
            alpha = sub_params_fit.query("param_name == 'v_Intercept'")["mean"].values[
                0
            ]
        if model_ID == 3:
            drift_gain = sub_params_fit.query("param_name == 'v_gain_plus_loss'")[
                "mean"
            ].values[0]
            drift_loss = -drift_gain
        else:
            drift_gain = sub_params_fit.query("param_name == 'v_gain'")["mean"].values[
                0
            ]
            drift_loss = sub_params_fit.query("param_name == 'v_loss'")["mean"].values[
                0
            ]
        if model_ID == 4:
            theta = 0
