...
def t_test_ind(params, 
    ...
    for param in params[param_name].unique():
        ttest = stats.ttest_ind(params.query(f"{param_name} == '{param}' & {condition_name} == '{conditions[0]}'")[param_value], 
                                params.query(f"{param_name} == '{param}' & {condition_name} == '{conditions[1]}'")[param_value])
